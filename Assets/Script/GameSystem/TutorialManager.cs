...
public class TutorialManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if((SelectManager.Instance.GetLatestGridObject().landType==LandType.Lake)&&(landlake<1))
        {
            ...
            tutorialUI.ShowMessageText("It will cause 2 action points stepping into water. But you can move without actionpoint in water. And it will only cause 1 action point to step out water.");
        ...
        if ((SelectManager.Instance.GetLatestGridObject().landType == LandType.Forest) && (landforest < 1))
        {
            landforest++;
            isActionCompleted = true;
            tutorialUI.ShowMessageText("Player in the forest won't be attack in attack stage");
            OnStartSpecificTutorial?.Invoke(this, EventArgs.Empty);
        }
