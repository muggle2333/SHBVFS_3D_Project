...
public class UIPanelInspector : UIRectEditor
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		{
			NGUIEditorTools.RegisterUndo("CardContent Alpha", mPanel);
			mPanel.alpha = alpha;
		}
		...
		GUILayout.BeginHorizontal();
		{
			...
			{
				NGUIEditorTools.RegisterUndo("CardContent Depth", mPanel);
				mPanel.depth = depth;
		...
		if (NGUIEditorTools.DrawHeader("Advanced Options"))
		{
			...
			GUILayout.BeginHorizontal();
			bool tool = EditorGUILayout.Toggle("CardContent Tool", mPanel.showInPanelTool, GUILayout.Width(100f));
